#!/usr/bin/env python
"""
Invoke mypy and compare the results with files in pytensor_ml.
...
FAILING = [
    line.strip()
    for line in (PROJECT_ROOT / "scripts" / "mypy-failing.txt").read_text().splitlines()
    if line.strip()
]
...
def check_no_unexpected_results(mypy_df: pl.DataFrame, show_expected: bool):
    ...
    """
    all_files = {
        str(fp).replace(str(PROJECT_ROOT), "").strip(os.sep).replace(os.sep, "/")
        for fp in PROJECT_ROOT.glob("pytensor_ml/**/*.py")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run mypy type checks on the pytensor_ml codebase."
    )
    parser.add_argument(
